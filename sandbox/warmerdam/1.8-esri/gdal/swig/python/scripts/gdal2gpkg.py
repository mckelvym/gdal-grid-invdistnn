...
This code was hacked together for testing GDAL GPKG driver.
- Not actually tested since GPKG driver isn't ready for use.
- Does not mosaic multiple files, adds single raster dataset with overviews if present.
- SR entry uses basename instead of proper organisation/code/id, etc.
- Uses pixel sizes from geo transform.

Based on other python utilities but made PEP compliant.
"""
__author__ = 'Robin Princeley'
__copyright__ = "Copyright 2013, Esri"
__license__ = "MIT"
    ...
    def add_dataset(self, filename):
        ...
        for overview_level in range(levels):
            ...
            if overview_band.XSize < self.tile_width or overview_band.YSize < self.tile_height:
                break
            level_pixel_x_size = pixel_x_size * (dataset.RasterXSize / overview_band.XSize)
            level_pixel_y_size = pixel_y_size * (dataset.RasterYSize / overview_band.YSize)
...
def usage():
    print("Usage: gdal2gpkg [--help-general] [-sr_org organization] [-sr_sysid identifier] [-sr_desc description]\n"
          "                 [-of format] [-co option]* datasetname gpkgname")
    return 2
...
def main(argv=None):
    ...
    while i < argc:
        if equal(argv[i], "--utility_version"):
            ...
            return 0
        elif equal(argv[i], "-sr_org") and i < argc - 1:
            i += 1
            gpkg.sr_organization = argv[i]
        elif equal(argv[i], "-sr_sysid") and i < argc - 1:
            i += 1
            gpkg.sr_organization_coordsys_id = int(argv[i])
        elif equal(argv[i], "-sr_desc") and i < argc - 1:
            i += 1
            gpkg.sr_description = argv[i]
        elif equal(argv[i], "-of"):
            i += 1
            if equal(argv[i], 'png'):
                gpkg.format = "image/png"
            elif equal(argv[i], 'jpeg'):
                gpkg.format = "image/jpeg"
            else:
                return usage()
        elif equal(argv[i], "-co"):
            i += 1
            gpkg.format_options.append(argv[i])
        elif argv[i][0] == '-':
            return usage()
        elif dataset_filename is None:
            dataset_filename = argv[i]
        elif gpkg_filename is None:
            gpkg_filename = argv[i]
        else:
            return usage()
        i += 1
    if dataset_filename is None or gpkg_filename is None:
        return usage()
